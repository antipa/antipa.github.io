function [X_den,iter]=hsvid_TV3DFista(Xobs,lambda,l,u,pars)
...
Ltv3 = @(P1,P2,P3)alpha*cat(1,P1(1,:,:),diff(P1,1,1),-P1(end,:,:)) + ...
    alpha*cat(2,P2(:,1,:),diff(P2,1,2),-P2(:,end,:)) + ...
...
% Preallocate
R1 = zeros(m-1,n,k, 'like', Xobs);
R2 = zeros(m,n-1,k, 'like', Xobs);
R3 = zeros(m,n,k-1, 'like', Xobs);
...
% Initialize
tkp1=ones(1,'like',Xobs);
count=0;
i=0;

D=zeros(m,n,k, 'like', Xobs);
%fun_all=zeros(1,MAXITER, 'like', Xobs);
st = 12*ones(1,'like',Xobs);
